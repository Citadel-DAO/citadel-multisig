...
interface IStakedCitadelLocker {
    ...
    function claimableRewardForToken(address _account, address _rewardToken)
        external
        view
        returns (StakedCitadelLocker.EarnedData memory userReward);

    function claimableRewards(address _account)
        external
        view
        returns (StakedCitadelLocker.EarnedData[] memory userRewards);

    function cumulativeClaimed(address, address)
        external
        view
        returns (uint256);

    function cumulativeDistributed(address) external view returns (uint256);
    ...
    function epochs(uint256)
        external
        view
        returns (uint224 supply, uint32 date);
    ...
    function lockedBalances(address _user)
        ...
        returns (
            ...
            uint256 unlockable,
            uint256 locked,
            StakedCitadelLocker.LockedBalance[] memory lockData
        );
